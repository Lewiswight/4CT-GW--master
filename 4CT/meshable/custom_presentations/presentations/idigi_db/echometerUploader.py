   ...
   #             
    ...
    def __send_to_idigi(self, data):
        ...
        try:
          ...
          #  file2.close()
            main_addr = "mainMistaway_" + gw_extended_address()
            website = self.cdb.channel_get(main_addr + ".website_address_com")
            post = self.cdb.channel_get(main_addr + "./address/posted/to")
            ...
            webservice = httplib.HTTP(website)
            #    print "line 1"
            webservice.putrequest(unicode("POST", "utf-8" ), unicode(post, "utf-8" ), "HTTP/1.1")
            #        webservice.putheader("POST", "/NumericUpDown.asmx", "" )
            #    print "line 2"
            webservice.putheader(unicode("Host", "utf-8" ), unicode(website, "utf-8" ))
